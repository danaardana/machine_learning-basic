# data/pengiriman.py - Shipping and Delivery

pengiriman_sentences = [
    # Basic shipping inquiries
    "Bagaimana cara pengirimannya?",
    "Bisa kirim ke luar Bandung?",
    "Ongkos kirim ke Jakarta berapa?",
    "Pakai ekspedisi apa untuk kirim?",
    "Bisa COD untuk area Bandung?",
    "Pengiriman ke Cimahi kena ongkir gak?",
    "Ada layanan antar di hari yang sama?",
    "Kirim pakai Grab atau Gojek bisa?",
    "Minimal order untuk free ongkir berapa?",
    "Bisa pickup sendiri di toko?",
    
    # Shipping options and safety
    "Packaging produk aman gak?",
    "Asuransi pengiriman tersedia?",
    "Kirim luar pulau bisa?",
    "Same day delivery tersedia?",
    "Estimasi sampai berapa hari?",
    "Ekspedisi yang dipakai apa aja?",
    "Ongkir ke Surabaya berapa?",
    "Bisa kirim via JNE?",
    "Delivery charge dalam kota berapa?",
    "Instant delivery ada gak?",
    
    # Tracking and documentation
    "Cargo darat atau udara?",
    "Resi pengiriman dikasih gak?",
    "Tracking online bisa?",
    "Door to door service ada?",
    "Express shipping tersedia?",
    "Pengiriman internasional ada?",
    "Layanan kirim luar negeri?",
    "Opsi pengiriman global?",
    "Jaringan pengiriman regional?",
    "Layanan pengiriman lokal?",
    
    # Coverage areas
    "Coverage pengiriman se-kota?",
    "Pengiriman pinggiran kota tersedia?",
    "Layanan pengiriman desa?",
    "Pengiriman lokasi terpencil?",
    "Pengiriman ke pulau tersedia?",
    "Pengiriman area pegunungan?",
    "Layanan pengiriman pantai?",
    "Pengiriman area perbatasan?",
    "Pengiriman zona ekonomi khusus?",
    "Pengiriman kawasan administrasi khusus?",
    
    # Specialized shipping services
    "Pengiriman zona bebas bea?",
    "Pengiriman area industri?",
    "Pengiriman area perumahan?",
    "Pengiriman district komersial?",
    "Pengiriman business park?",
    "Pengiriman shopping mall?",
    "Layanan pengiriman bandara?",
    "Pengiriman area pelabuhan?",
    "Pengiriman stasiun tersedia?",
    "Layanan pengiriman terminal?",
    
    # Logistics and distribution
    "Hub pengiriman network?",
    "Pengiriman depot tersedia?",
    "Layanan pengiriman warehouse?",
    "Pengiriman distribution center?",
    "Logistics hub delivery?",
    "Supply chain shipping?",
    "Cross-docking delivery?",
    "Layanan transshipment tersedia?",
    "Opsi consolidation shipping?",
    "Break-bulk delivery service?",
    
    # Load and capacity options
    "LTL shipping tersedia?",
    "FTL delivery service?",
    "Partial load shipping?",
    "Full load delivery?",
    "Container shipping tersedia?",
    "Pallet delivery service?",
    "Bulk shipping opsi?",
    "Unit load delivery?",
    "Temperature controlled shipping?",
    "Climate controlled delivery?",
    
    # Special handling
    "Refrigerated shipping tersedia?",
    "Frozen delivery service?",
    "Pengiriman bahan berbahaya?",
    "Dangerous goods delivery?",
    "Special handling shipping?",
    "Fragile item delivery?",
    "High value shipping?",
    "Secure delivery service?",
    "Insured shipping tersedia?",
    "Bonded delivery service?",
    
    # Customs and documentation
    "Customs clearance shipping?",
    "Duty payment delivery?",
    "Tax included shipping?",
    "Clearance service tersedia?",
    "Dokumentasi pengiriman lengkap?",
    "Bea cukai handling ada?",
    "Import documentation support?",
    "Export paperwork assistance?",
    "Compliance shipping service?",
    "Regulatory support delivery?",
    
    # Additional shipping variations
    "Pengiriman weekend tersedia?",
    "Holiday shipping service?",
    "Emergency shipping ada?",
    "Rush delivery tersedia?",
    "Scheduled delivery opsi?",
    "Flexible delivery time?",
    "Morning delivery tersedia?",
    "Evening delivery service?",
    "Night delivery opsi?",
    "24/7 layanan pengiriman?",
    
    # Payment and pricing
    "Cash on delivery tersedia?",
    "Credit card payment shipping?",
    "Bank transfer shipping?",
    "E-wallet payment delivery?",
    "Prepaid shipping opsi?",
    "Postpaid delivery service?",
    "Bulk discount shipping?",
    "Volume pricing delivery?",
    "Customer discount shipping?",
    "Regular rate delivery?",
    
    # Specific shipping capability inquiries
    "Kirim ke Bandung tersedia?",
    "Pengiriman ke Bandung bisa?",
    "Ke Bandung ada layanan kirim?",
    "Delivery ke Bandung support?",
    "Shipping ke Bandung available?",
    "Bandung masuk coverage area?",
    "Area Bandung dijangkau?",
    "Layanan ke Bandung ada?",
    "Bandung termasuk zona kirim?",
    "Coverage ke Bandung gimana?",
    
    # Order placement + shipping inquiries
    "Kalau pesan, kirim ke Bandung bisa?",
    "Order ke Bandung bisa diproses?",
    "Beli barang kirim Bandung bisa?",
    "Purchase ke Bandung tersedia?",
    "Checkout ke Bandung support?",
    "Transaksi ke Bandung available?",
    "Pembelian ke Bandung bisa?",
    "Shopping ke Bandung ada layanan?",
    "Belanja kirim Bandung gimana?",
    "Order online ke Bandung bisa?"
]

# Total: 120+ sentences for pengiriman intent
# Added patterns for shipping capability and order+shipping combinations 